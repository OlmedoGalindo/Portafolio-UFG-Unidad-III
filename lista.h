...
delete[] items;
 ...
 bool ListaEnteros::estaVacia(){ return this->cuenta == 0;

}

int ListaEnteros::tamanio(){ return this->cuenta;
...
for(int i=0; i < this->cuenta; i++){ this->items[i] = temp[i];
...
int valor = this->items [indice]; for(int i = indice; i < cuenta - 1; i++){ 
this->items[i] = this->items [i+1];
